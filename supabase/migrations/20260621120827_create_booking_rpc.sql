...
create trigger "ServiceCategory_set_updated_at" before update on public."ServiceCategory"
  for each row execute function public.set_updated_at();
...
create trigger "Client_set_updated_at" before update on public."Client"
  for each row execute function public.set_updated_at();
create trigger "WorkingHours_set_updated_at" before update on public."WorkingHours"
  for each row execute function public.set_updated_at();
create trigger "ScheduleBlock_set_updated_at" before update on public."ScheduleBlock"
  for each row execute function public.set_updated_at();
